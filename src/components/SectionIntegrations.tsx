...
function Pill({ src, alt }: { src: string; alt: string }) {
  return (
    <div className="shrink-0 rounded-full p-[1px] overflow-hidden [background:linear-gradient(180deg,rgba(255,255,255,.32),rgba(153,153,153,.10))]">
      <div className="inline-flex items-center gap-2 rounded-full bg-[linear-gradient(180deg,#0B1630_0%,#0A1834_100%)] px-4 py-2 ring-1 ring-white/10 shadow w-max">
        <Image
          src={src}
          alt={alt}
          width={90}
        ...
        />
        <span className="text-sm text-white/85 whitespace-nowrap">{alt}</span>
      </div>
    </div>
  );
}

function PartnersGrid({ items }: { items: { src: string; alt: string }[] }) {
  return (
